...
- 0.5.0 (2026-09-25, tagged `v0.5.0` on `release/0.3.0`) drops the flat preset shape: validation rejects it with the fields to move listed, `migratePreset()` converts old files once, and `FlatPreset`, `PresetInput`, `normalizePreset`, `flattenPreset` are gone from the package.
