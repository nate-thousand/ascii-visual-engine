...
- `release/0.3.0` branches from it and carries the 0.3.0 API work; `package.json` says 0.3.0. Done so far: `createEngine()` facade, nested presets (with flat normalization and a fixture backed compatibility test), HiDPI, `PointerInput`, frame budget (with a draw loop fix that took render from 12 ms to 3.3 ms at 1080p).
...
3. Continue 0.3.0 from `ROADMAP.md` section 2 in order: engine state (`idle | running | destroyed`), preset loader and export, params, effect tests.
