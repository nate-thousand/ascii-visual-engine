...
- `v0.2.0` is tagged at `12376f6` on `release/0.2.0`, `v0.3.0` at `46801ad` and `v0.4.0` at `0d49821` and `v0.4.1` at the head of `release/0.3.0`; branches and tags are pushed (2026-09-25). `main` is fast forwarded to the same head and the harness at visual-engine.xyz is deployed from it. Section 5 is demand driven: nothing there starts without a host asking. `npm install github:nate-thousand/ascii-visual-engine#v0.4.0` was verified from a scratch project on tag day: `prepare` builds `dist/`, CJS and ESM load, 30 presets.
- 0.5.0 (2026-09-25, local on `release/0.3.0`, package.json says 0.5.0) drops the flat preset shape: validation rejects it with the fields to move listed, `migratePreset()` converts old files once, and `FlatPreset`, `PresetInput`, `normalizePreset`, `flattenPreset` are gone from the package. Tag and push wait on the user.
